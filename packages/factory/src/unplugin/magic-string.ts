...
export class MagicString extends MagicStringBase {
  ...
  sliceNodes(nodes: Node[], { offset = 0 }: { offset?: number } = {}) {
    return this.slice(offset + nodes[0].start!, offset + nodes.slice(-1)[0].end!)
  }

  overwriteNode(node: Node, content: string | Node, { offset = 0, ...options }: OverwriteOptions & { offset?: number } = {}) {
