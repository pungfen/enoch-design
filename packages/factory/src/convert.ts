...
const index = <C extends FactoryConfig>(config: C) => {
  const origin: any = {}
...
const children = <C extends FactoryConfig>(config: C) => {
  const origin: any = {}
  if (config.children) {
    forEach(entries(config.children), ([key, value]) => {
...
export const block = <C extends FactoryConfig>(config: C) => {
  ...
  assign(origin, index(config), children(config))
